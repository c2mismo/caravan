...
void IRAM_ATTR flowmeterCounter(){
  if(initFlowmeterCounter)
  {
    ...
    endTimeFlowmeterCounter = xTaskGetTickCount();  //version de millis para interrupciones
    counterFlowmeter++;
...
int countOnTimer;
volatile int lastCountOnTimer;
int countCalc;


void IRAM_ATTR onTimer() {
  portENTER_CRITICAL_ISR(&timerMux);
  timeFlowmeterCounter = endTimeFlowmeterCounter - initTimeFlowmeterCounter;
  secFlowmeterCounter = timeFlowmeterCounter / 1000; // Segundos
...
void flowmeterCountTask1(void * pvParameters)
{
  ...
  while(1)
  {
    if(countOnTimer != lastCountOnTimer)
    {
      countOnTimer = lastCountOnTimer;
      countCalc++;
      Serial.print("on timer contados = ");
      Serial.println(lastCountOnTimer);
      Serial.print("Calculos contados = ");
      Serial.println(countCalc);
    }
